...
class MainHandler(BaseHandler,tornado.auth.FacebookGraphMixin):
    ...
    def get(self):
        ...
        if likes == 0:
            self.facebook_request("/me", self.get_likes, access_token=self.current_user["access_token"], fields="movies.fields(id,name),music.fields(id,name),favorite_athletes,favorite_teams,religion,political,sports,books.fields(id,name),games.fields(id,name),interests.fields(id,name),television.fields(id,name),activities.fields(id,name)")
...
class ScrapeHandler(BaseHandler, tornado.auth.FacebookGraphMixin):
    ...
    def sports(self):
        ...
        self.set_base_data(res, "favorite_teams", "favorite_athletes", "sports")
                ...
                if key in e:
                    ...
                    if user_gender:
                        for s in e[key]:
                            ...
                            #print "da"
                            pipe.sadd("likes:%s:%s" %(s["id"],user_gender, e["id"]))
        yield tornado.gen.Task(pipe.execute)
                ...
                if key in f:
                    ...
                    if user_gender:
                        ...
                        #print i.keys()
                            if "name" not in i:
                                continue
                            pipe.sadd("likes:%s:%s" %(i["id"],user_gender), f["id"])
        yield tornado.gen.Task(pipe.execute)
    ...
    def ready_data(self):
        ...
        for i in list(u_likes):
            ...
            if exists:
                ...
                for m in members:
                    pipe.sadd("matches:%s:%s" % (m, self.current_user["id"]), i)
                    l.append(m)
        yield tornado.gen.Task(pipe.execute)
        self.calculate(l)
